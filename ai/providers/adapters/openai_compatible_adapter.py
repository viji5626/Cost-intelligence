"""
Generic Local OpenAI-Compatible Provider Adapter
Enables optional connection to local-network/localhost OpenAI-compatible servers (e.g. vLLM, llama-server, LocalAI).
Strictly decoupled from AI-14 (which is the platform's own outbound /v1 serving endpoint).
"""
...
class LocalOpenAICompatibleAdapter(InferenceAdapter):
    """
    Provider adapter for generic local OpenAI-compatible inference daemons.
    """
    ...
    def __init__(
        ...
        super().__init__(name=name, provider_type=ProviderTypeEnum.OPENAI_COMPATIBLE, endpoint=clean_base, is_builtin=False)
    ...
    def update_endpoint(self, base_url: str, api_key: Optional[str] = None, **kwargs: Any) -> None:
        clean_base = base_url.rstrip("/")
        if clean_base.endswith("/v1"):
            clean_base = clean_base[:-3].rstrip("/")
        self.base_url = clean_base
    ...
    def translate_exception(
        ...
        if isinstance(exc, urllib.error.URLError) or "connection refused" in err_msg.lower() or "cannot connect" in err_msg.lower():
            return ProviderUnavailableError(
                message=f"Local OpenAI-compatible server unavailable at {self.base_url}: {err_msg}",
                provider_name=self.name,
                task_type=task_type,
                model_id=model_id,
                original_error_type=type(exc).__name__,
            )
        if "not found" in err_msg.lower() or "model" in err_msg.lower() and "exist" in err_msg.lower():
            return ModelNotFoundError(
                message=f"Model '{model_id}' not found on local server: {err_msg}",
                provider_name=self.name,
        ...
        if isinstance(exc, (TimeoutError, asyncio.TimeoutError)) or "timed out" in err_msg.lower():
            return ProviderTimeoutError(
                message=f"Inference timed out on model '{model_id}': {err_msg}",
                provider_name=self.name,
        ...
        if "oom" in err_msg.lower() or "out of memory" in err_msg.lower():
            return ProviderOOMError(
                message=f"OOM on model '{model_id}': {err_msg}",
                provider_name=self.name,
                task_type=task_type,
                model_id=model_id,
                original_error_type=type(exc).__name__,
        ...
        return AIProviderError(
            message=f"OpenAI-compatible provider error: {err_msg}",
            provider_name=self.name,
            task_type=task_type,
            model_id=model_id,
            error_class="OPENAI_COMPATIBLE_ERROR",
            original_error_type=type(exc).__name__,
        )
    ...
    async def generate_text(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            req = urllib.request.Request(url, data=data_bytes, headers=self._get_headers())
